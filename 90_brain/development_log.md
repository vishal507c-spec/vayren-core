...
## 2026-09-08 — Live screenshot fix: pill text clipping + edge cutoff (generic renderer)

**Kya hua:** Live chart par EOD pills `"OD 103.00 +1.7"` dikhe (leading "E" cut) + right-edge pill kata hua. Root cause (dono generic, strategy-blind): (1) pill measure-font vs draw-font mismatch — measure bold-7 se, draw ambient font se → pill text se chhota → `drawText` clip; fix: `_draw_placed_markers` poore call par marker font set karta hai (save/restore balanced). (2) pill x kabhi clamp nahi hota tha → `_layout_marker_pills(jobs, bounds=None)` mein optional chart-rect clamp (displaced flag honest). OBR/record/trading/contract untouched. **Tests:** +2 universal (full-text white-span, edge clamp + displaced) → 1518 pytest green; product chapters ruff/format/pyright 0, import/structure pass. NOTE: untracked `scripts/patch_research.py` (dusra workstream, toota hua) repo-wide ruff/pyright/ownership fail — unrelated, untouched. Push nahi (local only).

## 2026-09-08 — OBR exact reference visual (SL zero-visual, EOD/TIME split, muted bars)

**Kya hua:** Reference spec: SL bilkul invisible, EOD sirf OBR-classified exit par, TIME diamond barkarar. **Engine mein EOD concept nahi** (sirf EXIT_SL/EXIT_TIME) → OBR-owned adapter rule: TIME exit + entry same bar nahi → EOD blue arrow + `"EOD {p} {pnl}"`; entry same bar → TIME diamond (entry par EOD kabhi nahi); SL exit → koi plot nahi + `mute_signal_bar(exit)` taaki execution pill bhi na aaye. **Generic muted-bars channel (chhota, additive):** `PythonStrategy.mute/get_muted_signal_bars` → runner → `StrategyResult.muted_bars` (directional preserve) → TradeOverlay coverage union; renderer/chart untouched. **Record se SL visual code poori tarah hata** (level CREATE/UPDATE/COMPLETE, exit_sl marker, RenderLayer/PlotLifecycle imports); engine slPrice/SL trading 100% intact. **Tests:** §29a/b/c (SL-invisible+trading, SELL→EOD, same-bar TIME diamond), muted unit (base+overlay), runner muted forwarding, e2e → 1516 pytest green; product chapters ruff/format/pyright 0, import/structure pass. NOTE: untracked `scripts/patch_research.py` (dusra workstream, toota hua) repo-wide ruff/pyright/ownership fail — unrelated, untouched. Push nahi (local only).

## 2026-09-08 — OBR exact-match visual style (EOD blue arrow+label, solid pills, SL diamond)

**Kya hua:** Reference screenshot style: **generic renderer (no strategy branches):** naya `TRIANGLE_BLUE` marker (generic blue triangle primitive — EOD jaisa koi existing primitive nahi tha, isliye proven-necessary addition) + pills ab solid marker-color fill + white text (dark pill hata), directional triangles tip-anchored (pehle se), UP pills below-anchor / DOWN above (TradingView convention, screen-space only). **OBR owner:** TIME exit → EOD blue arrow + exact text `"EOD {price:.2f} {pnl:+.2f}"` (sirf exit candle par, plot_id `eod_N`); SL exit SQUARE → DIAMOND textless (Other Exit); entries/NT/REF/SL-level same. Trading ZERO change (pulses/prices/PnL same). **Tests:** parity EOD text/exit-diamond/e2e (white-text cluster = 1) + universal blue-triangle paint → 1510 pytest green; product chapters ruff/format/pyright 0, import/structure pass. NOTE: untracked `scripts/patch_research.py` (dusra workstream, toota hua) repo-wide ruff/pyright/ownership fail — unrelated, untouched. Push nahi (local only).

## 2026-09-08 — OBR marking visual redesign (reference-style attached markers + compact labels)

**Kya hua:** Reference screenshots (arrow tip candle se attached, chhota priced label) se match: **renderer (generic only):** directional triangles ab tip-anchored (tip exact logical price par, body bahar — screen-space only, logical coords untouched) + pill padding tight (+6/+2, radius 2, font/gap/size same); symmetric glyphs/colors/layers/LOD/batch sab same. **OBR owner text:** entries par `showEntryLabel=True` (default) → exact owner text `"BUY 1061.9"`/`"SELL …"` (renderer kabhi price generate nahi karta); False → text None; SL/TIME/NT/REF textless yathavat. **Dedup barkarar:** TradeOverlay bar-coverage se covered pills silent → ek event ek compact pill cluster (e2e pin). Trading ZERO change. **Tests:** +3 universal (tip-anchor up/down, compact pill geometry) + parity text-policy/e2e update → 1509 pytest green; product chapters ruff/format/pyright 0, import/structure pass. NOTE: untracked `scripts/patch_research.py` (dusra workstream, toota hua) repo-wide ruff/pyright/ownership fail — unrelated, untouched. Push nahi (local only).

## 2026-09-08 — OBR BUY/SELL visual conflict fix (generic ownership-aware suppression)

**Kya hua:** Asli duplication TradeOverlay se tha: backtest ke baad `trade_overlay.set_result(first)` har trade par ▲+BUY pill + dashed line paint karta, jabki strategy PlotEvent (buy_29 ▲ glyph-only) same bar par — box+marker combo. **Fix (generic, OBR-blind):** `06_backtest/backtest/ui/overlay.py` mein `_strategy_marker_bars()` (duck-typed MARKER bar extraction, REMOVED/HIDDEN/de malformed safe) + `TradeOverlay.set_result` coverage per result + paint mein covered entry/exit marker+pill skip; connection line (holding span, alag info) hamesha; focused-trade detail hamesha full; `clear()` coverage reset. Koi strategy naam, koi signal semantics, koi chart import, koi renderer change nahi. **Tests:** `test_ui.py` +10 (generic paint intact, suppression+connection, partial/unrelated/removed/non-marker/malformed, per-result reset, focused full, determinism, no-branch source scan) + parity mein OBR e2e (runner→TradeOverlay: covered {24,29,30}, pills 0, connection paints) → 1506 pytest green. OBR trading/parity/universal sab unchanged. NOTE: untracked `scripts/patch_research.py` (dusra workstream, toota hua) repo-wide ruff/pyright/ownership fail — unrelated, untouched. Push nahi (local only).

## 2026-09-08 — OBR visual migration: legacy markings → universal PlotEvents (owner-side)

**Kya hua:** Screenshot duplicate (`BUY 64.90` pill + universal marker same event) ka root cause: OBR record sirf legacy `plot()`+meta series emit karta tha (universal path khali), aur base ka `_mirror_legacy` universal events ko wapas legacy series mein duplicate karta — dono PlotOverlay mein paint hote. **Migration (owner-side only):** canonical record (`D:\VAYREN_STRATEGIES\OBR.py`) ka `_plot_obr_visuals` ab universal API use karta hai — BUY→UP_ARROW, SELL→DOWN_ARROW, EXIT SL→SQUARE, EXIT TIME→DIAMOND, NO TRADE→CIRCLE_X (sab text None, exact engine bar/price, `view.index`+`view.bar.symbol`), REF→`plot_ray(extend_bars)` (visual-only), SL→`plot_level` CREATE@entry→UPDATE per bar→COMPLETE@exit (`_sl_plot_id` se); legacy `_marker` helper + `plot()` calls + `_plot_meta` extend writes sab hata diye; PNL text markers drop (price-text ban). **`base.py`:** duplicate-producing `_mirror_legacy` + `_legacy_marker_kind` delete (legacy `plot()`/`get_chart_series()` path intact for direct callers). Trading ZERO change (pulses/signals/PnL same; `showEntryLabel` cfg retained, visuals ab glyph-only). **Tests:** parity visual asserts universal par rewrite (exact bar/price, one-event-one-plot, no PNL/legacy, extend/refIndex preserved) → 72 green; universal 28 green unchanged. **Proof:** full pytest 1495 green, product chapters ruff/format/pyright 0, import/structure pass. NOTE: untracked `scripts/patch_research.py` (dusre workstream ka, ab toota hua) repo-wide ruff/pyright/ownership fail kar raha hai — unrelated, untouched. Push nahi (local only).

## 2026-09-08 — OBR NO-TRADE visual migration (owner-side, CIRCLE_X glyph-only)

**Kya hua:** Sirf canonical OBR record (`D:\VAYREN_STRATEGIES\OBR.py`, `_plot_obr_visuals`) ki ek line badli: `self._marker("NO TRADE", eng.ntEventVisPrice, "marker_square", "NO TRADE")` → `self._marker("NO TRADE", eng.ntEventVisPrice, "marker_circle_x", "none")`. Trading logic ZERO change (ntEventNow/ntEventVisPrice engine se same; series `{25: 101.24625}` bar+price identical; entry/exit/SL/TIME/PnL/day-type sab same). Universal infra (`plot_event.py`, `plot_renderer.py`, PlotStore/Overlay, runner/bootstrap forwarding) bilkul untouched — `"marker_circle_x|none"` pehle se supported glyph-only path hai. **Tests:** parity file mein style assertion update + naya `test_visual_no_trade_is_circle_x_without_text` (CIRCLE_X + fmt None + exact bar/price + negatives: no square/label/duplicate/shift) → 72 green; universal 28 green unchanged. **Proof:** full pytest 1495 green, ruff/format/pyright 0 (mere files), import/structure pass. NOTE: `scripts/patch_research.py` (untracked, kisi aur workstream ka) ruff E501 ×8 + unclassified de raha hai — is task se unrelated, chhuta nahi. Push nahi (local only).

## 2026-09-08 — Universal strategy-owned plotting + extreme performance architecture

**Kya hua:** Strategy owns meaning, pipeline transports, renderer only draws — OBR trading logic ZERO change (engine `process_bar` untouched; parity file 71 green unchanged). **Contract (`05_strategy/strategy/models/plot_event.py`, naya — STRATEGY/Python-owned):** frozen `PlotEvent` (event_id/source_strategy/plot_id/symbol/timeframe/plot_type/bar|start|end/price|start|end/timestamp/marker/text/layer/lifecycle/version/extend_bars/dependencies/metadata) + `PlotType`×10 (LINE/RAY/SEGMENT/MARKER/SHAPE/LABEL/ZONE/HORIZONTAL_LEVEL/VERTICAL_MARK/AREA) + `MarkerType`×9 (UP_ARROW/DOWN_ARROW/CIRCLE/CIRCLE_X/SQUARE/DIAMOND/TRIANGLE_UP/TRIANGLE_DOWN/DOT) + `PlotLifecycle` + generic `RenderLayer` + deterministic `make_event_id` + `with_update` versioning + `to_dict/from_dict` serialization; text default None (koi pill nahi jab tak strategy na maange); coordinates logical (bar+price) exact, pixel math sirf renderer mein. **Strategy API (`strategies/base.py`, extend):** `plot_marker/line/ray/segment/zone/label/level` + `update_plot/remove_plot` + `get_plot_events` — visual-only, trading API (`buy/sell/stop_loss/...`) se fully separate (plot kabhi trade nahi, trade kabhi plot nahi); same-bar multiples unique event_id se coexist; legacy `_plot_series/_plot_meta` mirror barkarar (runner/PlotOverlay path unchanged); `compiler.create_logic` ab `owner_id` set karta hai (source_strategy). **Store+renderer (`04_chart/.../plot_renderer.py`, existing file, zero nayi Qt surface):** Qt-free `PlotStore` (compact immutable records; strategy/symbol/timeframe/type/lifecycle + bar-sorted index; `ingest/ingest_batch` atomic frame, `update/remove/remove_strategy`, `query_visible` index-based, dirty-region, measured telemetry, malformed skip — error isolation) + `PlotOverlay` universal ingest (`ingest_plot_events/update_plot/remove_plot/total+visible counts/stats`) + layer-ordered batched paint (zones→levels→lines→markers→labels; markers global de-collision mein; glyph-only jab text None; text-measure cache; LOD: extreme zoom-out par fills→lines, glyphs kabhi drop nahi) + 5 naye glyphs (circle/circle_x/diamond/triangle_up-down/dot) + `marker_x|none` glyph-only style; legacy `_marker_priority` titles + line/ray paths byte-identical. **Unification (`backtest`):** `StrategyResult.chart_plots` (defaulted, duck-typed Any) + runner `get_plot_events` forward + directional preserve; bootstrap backtest-completed + live `_run_strategy_plots` dono paths ingest karte hain — LIVE/BACKTEST/REPLAY ek hi contract. Pan/zoom/resize/visibility sirf filter/re-query (koi recalc nahi). **Tests:** naya `05_strategy/strategy/tests/test_universal_plots.py` 28 green (contract×7, strategy API×5, store×5, renderer×7 incl. no-strategy-branch source scan + exact-pixel + hidden-filter, OBR acceptance×2, perf 100k store + 20k paint). **Proof:** full gate green (1494 pytest + 49 scripts, ruff/format/pyright 0, validators incl. language-ownership PASSED; `plot_event.py`=STRATEGY, test=TEST warnings only). Push nahi (local only).

## 2026-09-08 — RESEARCH LAB institutional workstation (engine-backed, no fakes)

**Kya hua:** Static Research screen → hypothesis-first quant workstation, same app shell. **Engine (Python, existing files):** `strategy/research/analysis.py` ab trade-sequence se real drawdown (cumulative-PnL peak-trough), per-trade Sharpe (mean/std, ≥2 trades), Sortino (downside deviation), payoff ratio compute karta hai; CAGR/volatility/exposure/turnover dataset mein equity-curve/bars nahi → honest None (UI mein N/A). **Service (`app/services/research_service.py`, extend):** `AnalysisView` + status (READY/RUNNING/COMPLETE/FAILED/NO DATA/NO TRADES) + 17-metric dict + trades/signals/data-quality/reproducibility; naye honest helpers `data_quality` (missing/duplicate = NOT CHECKED), `reproducibility` (experiment/strategy/dataset/params/TF/range), `run_robustness`/`run_validation` (real `strategy.research`), `comparison` (NO RESULT jab result nahi). **UI (`app/ui/research_workspace.py`, extend, zero nayi file):** command bar (dataset/universe/strategy/version/TF/period/experiment + RUN + status badge), center mein HYPOTHESIS → 17 METRICS → tabs (SIGNALS 8-col sortable/filterable + TRADES + trade inspector entry/exit/side/reason/PnL/hold + MFE/MAE N/A + ROBUSTNESS + COMPARE + DATA + VISUALS empty-states), right EXPERIMENTS inspector (config/data/result/validation/reproducibility), bottom real-action log + engine badge. Khali state = RESEARCH READY, metrics N/A, No signals/experiments. OBR untouched (Strategy Lab mein hi, research sirf consume karta hai). **Tests:** existing file mein +8 (17-metric N/A honesty, NO TRADES status, quality/repro/robustness/validation/comparison/layout/empty) → 15 green. **Proof:** full gate green (1466 pytest, ruff/format/pyright 0, validate_imports/structure pass); `validate_language_ownership` ka 1 warn pre-existing hai (untracked `test_obr_pine_parity.py`, is task se unrelated — nayi file ZERO). Push nahi (local only).

## 2026-09-08 — OBR REF index + extension params (visual/param fix, logic frozen)

**Kya hua:** Trading logic ZERO change (engine `process_bar` untouched; 6 AMBUJACEM trades identical). **refIndex:** engine hamesha se `candleCount == refIndex-1` use karta tha — ab tests pin karte hain refIndex=1/3/4/5 se REF candle + setup move hota hai (visual REF origin + engine entry dono). **extendBars:** naya user param (default 5, min 1, max 20) sirf visual REF line length chalata hai (`bars:N` meta); engine default bhi 7→5 align (engine field ko `process_bar` kabhi padhta nahi — patch-time assert). extendBars=3/5/7 par trading identical (field-wise trade compare). REF rays teal/red pixel-pinned. **Tests:** existing parity file +10 → 71 green. **Proof:** full gate green (1458 pytest, ruff/format/pyright 0, validators); live smoke: 6 params, REF `bars:5`, SL `none`, 6/6 SIGNAL-close. Markers mein koi change nahi (is task ka scope REF/extension/refIndex). **Naya file ZERO.** Push nahi (local only).

## 2026-09-08 — OBR chart visual cleanup: short levels + de-collided labels

**Kya hua:** Trading behaviour ZERO change (engine untouched; 6 AMBUJACEM mirror trades side/time same; 53 parity tests green). **Short levels:** renderer mein generic `extend="bars:N"` — har ray origin bar se max N candles (OBR REF HIGH/LOW = `bars:5`, meta runner se forward hoti hai, koi runner/model change nahi); SL line `extend="none"` — sirf active bars tak exact (koi trailing overhang nahi); din alag-alag (gap 25 ≫ cap). Legacy `session` rays unchanged (contrast test). **Labels:** marker pills ab saari marker series mein ek saath layout hote hain — deterministic priority (BUY/SELL → EXIT → NO TRADE), glyph exact rehta hai, pill sirf vertically nudge hota hai (+ stem jab displaced ho); undisplaced output pixel-identical. **Tests:** existing parity file mein +9 (extend-meta, parse contract, capped-ray/uncapped/SL-no-overhang QImage paints, layout unit, same-bar paint, priority) → 62 green. **Proof:** full gate green (1449 pytest, ruff/format/pyright 0, validators); live smoke 250 bars: REF gaps 25–26 par rays capped, 6/6 SIGNAL-close. **Naya file ZERO.** Push nahi (local only).

## 2026-09-08 — OBR Strategy Lab chart: TradingView-like visuals (engine read-only)

**Kya hua:** Trading behaviour ZERO change (`OBREngine.process_bar` untouched; pichhle smoke wale 6 AMBUJACEM trades side/time same). **Adapter** (`D:\VAYREN_STRATEGIES\OBR.py`, existing file): `_plot_obr_visuals` naya — sirf engine pulses/state plot karta hai (koi OBR condition recompute nahi, koi `self.stop_loss()` nahi): BUY/SELL marker (entryEventPrice, exact entry bar — RAW pulse se, taaki 0-hold same-bar round trip bhi dikhe), SL line (eng.slPrice: LONG=REF LOW/SHORT=REF HIGH, active bars + exit bar tak), EXIT SL/TIME + PNL (visPrice/pnl), NO TRADE (ntVisPrice); style/format series-meta mein (`marker_up|BUY {v:.2f}` …), jo runner pehle se forward karta hai. **Renderer** (`04_chart/.../plot_renderer.py`, existing file, additive): `marker_up/down/square` style par glyph + label pill (theme colours, fixed `{t}/{v}` token format, koi eval nahi); `line` path byte-identical. REF HIGH/LOW rays jaise the waise (session ray, din alag-alag). **Tests:** existing `test_obr_pine_parity.py` mein +12 (series values/bars/meta, runner style-forwarding hermetic test, headless QImage marker + line-path tests) → 53 green. **Proof:** full gate green (1440 pytest incl. 169 app, ruff/format/pyright 0, validators); live smoke AMBUJACEM 250 bars: BUY×3/SELL×3/SL-line/EXIT-SL×4+PNL/EXIT-TIME×2+PNL/NT×4, 6 mirror trades sab SIGNAL-close. **Naya file ZERO** (core mein koi nayi file nahi; sirf existing files extend). Push nahi (local only).

## 2026-09-08 — OBR Pine → Strategy Lab 1:1 parity audit (no redesign)

**Kya hua:** Original Pine source file (desktop `OBR.txt`) empty mili — isliye task-spec ke verbatim Pine rules (operators, layer order, formulas) ko spec maankar canonical Lab implementation (`D:\VAYREN_STRATEGIES\OBR.py` JSON record: StrategyConfig/Bar/TradeRecord/OBREngine/Strategy) ka audit kiya. **Engine verdict:** LAYER 0–12 order, strict breaks (`low < refLow`, `high > refHigh`), double-break invalidation bina tie-break, resolver (both→NONE), direction lock, same-bar entry, LONG SL=REF LOW (`low <=`) / SHORT SL=REF HIGH (`high >=`), 15:15 time exit bar-OPEN par, NO-TRADE record, end-of-bar reset, IST days, slippage/commission/PnL formulas, equity base 100000.0, maxDays=1000 rolling — sab Pine-spec ke mutabiq, engine untouched. Dataset-open counting (`prev None` → count 1, ref second bar day-1 par) Pine ke `ta.change()=na` transliteration ke roop mein verbatim preserve (test mein pin; redesign mana hai). **2 adapter fix (engine untouched, thin barkarar):** (1) same-bar entry+exit pulse par koi Lab signal nahi — pehle BUY emit hota tha jabki engine flat/dayDone tha (phantom position + mirror desync cascade); ab mirror Pine end-state (flat) se match, engine record mein 0-hold trade safe; (2) `_engine_bar` tz: naive→IST attach, aware→convert (pehle `replace()` offset overwrite karta tha). **Tests:** `05_strategy/strategy/tests/test_obr_pine_parity.py` — 41 tests (ref/break/confirm/double-break/invalidation/resolver/lock/entry/SL/time-exit/NT/reset/multi-day/analytics/rolling/prefix-invariance/adapter mapping/warmup-0/5-params/REF plots), sab green. **Proof:** full gate green — pytest 1597 (1428 + 169 app), ruff/format/pyright 0, validators pass; live smoke `BacktestRunner` AMBUJACEM 15m 2026-05-01→15: 250 bars, 6 mirror trades sab SIGNAL-close (koi stranded position nahi), REF HIGH/LOW 10 sessions plot. Repo ka `05_strategy/.../obr.py` (Obr/ObrSellV10 RSI strategies) alag cheez hai — untouched. Push nahi kiya (local only).

## 2026-09-07 - Release tooling: 30-second `scripts/release.py`

**Kya hua:** Release mein time validation gate mein lagta tha (~3-5 min), push khud seconds ka hai. `scripts/release.py --version X.Y.Z` banaya: version bump → stage (scratch excludes) → staged-diff secret scan → commit → annotated tag → push branch+tag → `gh release` attempt (auth fail par exact recovery command). Full suite dobara nahi chalata (release se pehle `make check`, push par CI mirror karta hai). `make release VERSION=x` target bhi. End-to-end disposable-copy mein proven: commit+tag 0.5s. Docs: AGENTS-style safety preserved (fail-closed on secrets/dirty-tag).

## 2026-09-07 - FINAL: autonomous product quality pass (terminal UI + research)

**Kya hua:** Full-product audit → issue inventory (rail clipping, tooltip-only gate reasons, per-second row rebuilds, KV overflow, hardcoded splitter, unsized tables, disabled Research/Portfolio nav, dead regime wiring hunt, stale toolbar docs, dead `_PlaceholderView`). **UI kit:** `app/ui/ui_kit.py` (Section/Badge/KVBlock/GateRow/tables/EmptyState, shared tokens; lab_theme stays the palette source). **LIVE rebuild:** scroll-safe rail, visible Gate/Status/Reason rows with widget reuse (no per-tick churn), wrapping KV, proportional splitter, sized tables with content-hash skip + cell tooltips, enriched NO DATA state (symbol/timeframe/source/action). **Research:** `services/research_service.py` (datasets from backtest histories, experiments, real `analyze_dataset` runs) + `ui/research_workspace.py` (navigator/analysis/inspector/log, honest empties). **Portfolio:** `ui/portfolio_workspace.py` over the shared workspace-state dict (funds/account/allocation/P&L/orders/fills; missing funds = UNAVAILABLE, P&L total rejected as equity basis). **Nav/window:** RESEARCH + PORTFOLIO enabled with signals, stack slots + show methods, bootstrap wiring. **Proofs fixed:** equity-basis bug + unidentified-history bug caught by new tests; stale toolbar docs corrected; dead `_PlaceholderView` removed; regime panel confirmed honest-unknown (docstring fixed). **Responsive:** offscreen geometry harness at 1280×720→2560×1440 (38 labels + tables + controls per size, all green). **Tests:** +61 (kit/nav/research/portfolio/responsive). **Proof:** full gate green (details in final report). Koi future phase nahi.

## 2026-09-07 - FINAL: language migration engine (Rust ownership + egui target)

**Kya hua:** FINAL language pass — toolchain proven (cargo 1.98 + MSVC link, probe builds), full audit (`scripts/language_audit.py` → `90_brain/language_inventory.json`, 491 files classified, 0 unclassified). **Rust workspace `rust/`:** `vayren-core` cdylib (std-only, zero deps) owns 3 authorities — order lifecycle table (`order_state.rs`), backtest numeric kernels (`metrics.rs`: drawdown/equity/sharpe), timeframe aggregation + mode (`aggregate.rs`, `stats.rs`) — 27 Rust unit tests green; `vayren-shell` egui crate compiles (broker panel view-model, 4 headless tests, 4MB binary). **Boundary:** `core.native.loader` (fail-closed ctypes handshake) + thin bridges (`native_order_state`, `native_metrics`, `native_aggregate`); Python math DELETED (no duplicate authority); `TRANSITIONS`/`TERMINAL_STATES` re-exported from Rust projection (public paths unchanged); engine UNKNOWN-bypass folded into table (parity caught CREATED/VALIDATED→UNKNOWN nuance, preserved). **Parity:** 13×13 frozen oracle + seeded fuzz (200-300 cases/kernel) + edge cases, all green. **Perf (measured, median-of-7):** drawdown 100k 4.7×, sharpe 20k 4.1×, aggregate 60k 1.5× (kernel 8×, FFI-bound end-to-end), equity ~parity (output-bound), transitions neutral. Zero-copy `array`+`from_buffer` marshalling after honest 50× ctypes-tuple finding. **Validator:** `scripts/validate_language_ownership.py` (baseline 491 files, anti-reintroduction AST, crate hygiene) wired into `make check` + CI + gate; Qt UI proven-retained (big-bang forbidden), strategy/AI/research Python-owned, `language_retention.json` carries proofs. **EXE:** spec bundles the cdylib (guarded, loud if missing). **Proof:** full gate green (details in final report). Koi future language-migration phase nahi.

## 2026-09-07 - FINAL: production broker architecture (no future phases)

**Kya hua:** FINAL engineering phase — M6/M7/M8 tree source of truth, koi future architecture phase nahi. **UBL freeze:** `broker/identity.py` (typed `BrokerIdentity` broker_id/display_name/environment/account_id + `identity_of`); credentials lifecycle (`CredentialRef.broker` bound refs + `expected_broker`/`expected_scope` mismatch fail-closed, `CredentialMetadata` expiry/rotation + `validate_metadata`); `surface_status` empty-set→NOT_CONFIGURED. **Market data:** `MarketDataFace` full lifecycle (connect/disconnect/reconnect + auth-via-resolver doc) + `ReplayProvider` impl; `StreamNormalizer` malformed-event fail-closed (`MarketDataError`). **Account/funds:** `AccountSnapshot` += account_id/environment (defaults compat); `gates.py` `funds_snapshot_from_face` (dual UBL/execution shape) + `risk_capital_from_funds` (available→available_capital, equity→equity) + `funds_valid_for_live`; Paper/Sandbox tuples honestly advertise `account.funds` (M6 impl, no new constant). **Idempotency:** `ExecutionEngine.snapshot/restore` + checkpoint/recover (`IDEMPOTENCY_RESTORED`, legacy checkpoints compat); UNKNOWN still reconcile-only. **Lifecycle:** `RECONCILING` state (RECOVERING→{VALIDATING,RECONCILING} compat); `session.start` recovers via `reconcile_now` before VALIDATING; journal chain += broker/environment on PLANNED/SUBMITTED/ACK/FILL. **Reconcile:** `ReconcileStatus` SAFE/WARNING/BLOCKED + `reconcile_funds` + `ReconciliationVerdict`/`verdict_of`/`record_verdict` + `State.funds`/`verdict()` (paper economics documented: ledger-vs-venue funds differ by design, funds verdict via ceremony). **Resilience:** `TimeoutPolicy`/`BackoffPolicy`/`ReconnectPolicy` (bounded, deterministic). **Skeleton:** `adapters/skeleton/` reference template (all faces fail-closed, transport NOT_CONFIGURED) + future-broker registration proof. **Ceremony:** `broker/activation.py` 12-step `evaluate_activation` + `record_activation` (pure verifier, START never auto-ok). **Exports:** broker/execution/portfolio `__init__` updated; structure requires identity/skeleton/activation. **Tests:** +36 (`test_final_phase` 12, `test_final_production` 19, arch +5 → 20 proofs). **Proof:** 1344 pytest, 28/28, ruff/format/pyright/validators clean, `--paper` RC=0 CLEAN, `--check-live` NOT READY RC=0, security clean. Benchmarks: dispatch ~709k ev/s, risk 0.017ms, reconcile 0.015ms, paper bootstrap ~43ms. Baaki SIRF external operational: broker account, credentials, permissions, KYC, endpoint, operator LIVE consent. Koi M9+ nahi.

## 2026-09-07 - Phase 25: UBL M8 — production broker adapter framework (M6/M7-compatible)

**Kya hua:** M8 execute — current M6/M7 tree source of truth (M6 `funds.py` + Paper/Sandbox `funds()` reused verbatim, M7 retired `register_provider`/`register_adapter` untouched, retained `build_provider`/`resolve_broker`/`Provider`/`BrokerAdapter` preserved). **Naya:** `broker/capabilities.py` tri-state `CapabilityStatus` (SUPPORTED/NOT_SUPPORTED/NOT_CONFIGURED) + `CapabilitySet.status()` + `capability_status()` (bool helpers unchanged); `broker/selection.py` `surface_status()` + `BrokerSelectionService.surface_status()` (bool `surface_allowed` compat); `broker/credentials.py` (CredentialScope HISTORICAL/TRADING/MARKET_DATA, value-less `CredentialRef` redacted, `CredentialResolver` protocol, env-aware `validate_refs` — existing stores implement structurally, no data↔execution cycle); `broker/health.py` (`HealthState` 8 generic states + frozen `BrokerHealth` + `to_legacy`/`health_from_legacy` compat; health≠LIVE readiness); `faces.py` `TradingFace.place_order(..., idempotency_key=None)` optional + `MarketDataFace.subscribe/unsubscribe`; `order.py` `MODIFY_PENDING`/`MODIFIED` (additive, UNKNOWN still reconcile-only); `resilience.py` `MUST_RECONCILE_FIRST` alias + M8 policy doc; `ReplayProvider.subscribe/unsubscribe`; validators: `NETWORK_DENYLIST` (kiteconnect/httpx/requests/websockets/websocket, allowlist adapters+provider, 99_archive excluded) + structure requires credentials/health. **Tests:** +44 (`test_adapter_contract.py` 38 generic harness: identity/tri-state/historical/market-data/funds/health/orders/cancel/modify/fills/disconnect/unknown/reconcile/idempotency/errors/redaction/retry/env/SDK-isolation; arch rules +6 → 15 proofs: network-per-chapter/SDK-types/one-selection+vocab/research-free/no-creds-in-UI/face-fail-closed). Zerodha history SUPPORTED, trading/funds NOT_SUPPORTED; UI unchanged (domain ✓/✗ already honest). **Proof:** 1308 pytest green, 28/28 partitions, ruff/format/pyright/validators clean, `--paper` RC=0 CLEAN, `--check-live` LIVE NOT READY RC=0, security scan clean. LIVE = NOT_CONFIGURED (correct).

## 2026-09-07 - Phase 24: UBL M7 — legacy broker/provider shim retirement (scoped, behavior-preserving)

**Kya hua:** Scoped M7 execute. **Discovery:** repo-wide shim search `M7-shim-baseline.txt` mein freeze (312 lines); funds M6 se present tha (Paper/Sandbox `funds()` + `broker/funds.py`), `--paper` RC=0 CLEAN, `--check-live` fail-closed RC=0, LIVE = NOT_CONFIGURED. **Direct UBL migration:** `bootstrap.py` ka `build_provider(data_settings)` caller hata — ab `default_registry().get(selection.name).plugin.face(HISTORICAL_DATA, ...)` seedha (same `surface_allowed` guard + `unavailable_provider` fail-closed; `DownloadSettings(provider=selection.name)` derived barkarar); engine TypeError/status docstrings ab UBL registry ki taraf point karte hain; `settings.provider` documented derived (INVARIANT: hamesha `BrokerSelection.name`, ONLY write path Bootstrap). **Retired (proof-based):** `register_provider` + `register_adapter` delete (+ `ProviderFactory`/`_AdapterFactory` dead code, `execution` re-exports); tests/bench (`test_provider`, `test_shims`, `test_sandbox_e2e`, `test_live_safety`, `bench_execution`) ab direct `BrokerRegistry.register` use karte hain. Duplicate-name semantics ab fail-closed (`DuplicateBrokerError`) — shim wali overwrite leniency design se gayi. **Retained (evidence-backed):** `build_provider` (compat delegate), `resolve_broker` (koi UBL trading-resolve helper nahi — banane se execution/risk redesign hoga), `Provider`/`BrokerAdapter` (face-swap abhi scope se bahar), `DownloadSettings.provider` (derived compat), transports (Zerodha transport M5-decision se `02_data` mein — SDK isolation; Paper/Sandbox venues `08_execution` mein). **Validators:** SDK allowlist mein `09_broker/broker/adapters/` future boundary; arch tests +5 (M7-A–E: no build_provider callers in app, zero register_* refs, home-chapter concrete imports, bootstrap-only provider writes) + branching scan mein `00_app`. **Tests:** +20 `test_m7_direct_ubl.py`; `test_shims`/`test_provider` migrate (koi delete/disable/loosen nahi). Paper/Sandbox/gates/strategy/backtest untouched.
**Baaki:** UBL trading-resolve helper + credentials model (phir `resolve_broker` retirement), `Provider`/`BrokerAdapter`→UBL-face swap (Zerodha transport `02_data` mein hi rehta hai — M5-documented boundary), phir `build_provider` retirement. LIVE = NOT_CONFIGURED (correct).

## 2026-09-07 - Phase 23: UBL M6 — funds/margin capability (cash-only, fail-closed)

**Kya hua:** Approved design D8 + locked M6 decisions execute. **Naya:** `09_broker/broker/funds.py` (UBL-owned frozen `FundsSnapshot`: available/used/equity + currency/account_id/timestamp, strict `to_dict` exact-3-keys/`from_dict`, NaN/negative/inf/extra-key rejection, legitimate `0.0` preserved; module sentinels `FUNDS_UNKNOWN`/`FUNDS_UNSUPPORTED` + `is_unknown`/`is_unsupported` — UNKNOWN≠UNSUPPORTED≠ZERO; `require_funds(face)` capability gate → existing `UnsupportedCapabilityError`, never calls `funds()` when unsupported, never fake zeros; future risk map documented `available→available_capital`, `equity→equity`, docs-only). **Paper/Sandbox:** `funds()` added (cash-only `available==equity==live _capital`, `used==0.0`, `FundsSnapshot(...).to_dict()`; fill/commission/capital economics untouched; Sandbox carries `account_id`). **Factory:** `_PAPER_CAPS/_SANDBOX_CAPS` += `account.funds` via existing `_LEGACY_TO_UBL` mechanism (byte-identical UBL id, NO new `BrokerCapabilities` constant — no second vocabulary); Zerodha untouched history-only. **Registry:** `find_with(ACCOUNT_FUNDS)` → paper+sandbox, zerodha excluded; selection flow unchanged.
**Tests:** +35 (`test_funds.py` 31: contract A1-10/capability B11-13/paper C14-19/sandbox D20-23/unsupported E24-25/registry F26-27/safety G28-31; `test_shims.py` 1 assertion flipped to M6 expectation; `test_architecture_rules.py` 4: UBL-ownership/no-SDK-imports/no-second-vocabulary/funds≠LIVE). **Compatibility:** `TradingFace.funds()` signature unchanged, RiskEngine/gates/adapter/Zerodha/UI/strategy/backtest/research/shims untouched; LIVE = NOT_CONFIGURED.
**Baaki:** M7 shim retirement; real-broker venue + credentials + account confirmation + risk wiring.

## 2026-09-07 - Phase 22: UBL M5 — Zerodha adapter migration (M5 ONLY)

**Kya hua:** Approved design §5/M5 execute. **Naya:** `09_broker/broker/adapters/__init__.py` (boundary rules) + `09_broker/broker/adapters/zerodha/__init__.py` (`BROKER_ID="zerodha"`, `DISPLAY_NAME`, `HISTORICAL_CAPABILITIES` matrix history-only, `zerodha_plugin_record()` builder with injected factory — no data/SDK imports). **Transport untouched:** auth/fetch/instruments/credentials/KITE_INTERVAL_IDS exactly once in `02_data/data/provider/zerodha/` (koi move/copy nahi — move karne se broker→data import cycle + validator violation hota; deviation design doc §4 se documented). **Additive only:** `ZerodhaProvider.name` + `capabilities()` (structurally HistoricalFace). **Single identity:** factory seed + settings default + selection default sab `BROKER_ID` constant se (3 literals khatm). **Registry:** zerodha = history-only record; trading/market-data/funds faces absent → fail closed. **Validators:** structure list me adapters entries; SDK allowlist UNCHANGED (update required nahi tha).
**Tests:** +16 (`test_zerodha_adapter.py`: identity/aliases/matrix/registration/faces/duplication/isolation-subprocess/boundary/selection-E2E/live-safety/credentials). **1204 total**, 28/28 partitions, ruff/format/pyright/validators clean. Paper E2E CLEAN (PnL -572.76, selection sandbox line), check-live fail-closed NOT READY (gates evaluated, sandbox≠live).
**Measured:** full pytest 110.2s, run_tests 204.6s, wall 5805.5s (~96.8 min). 6 failing assertions / 2 red runs / 4 repair rounds → first-pass False. Humans 0. Speedup: NOT MEASURED (no equivalent prior fingerprint).
**Housekeeping:** M4 smoke-test ka stray `broker_selection.json` (sandbox) real data dir se hataya; journal failures 4→6 correct kiya (declared, scoreboard se pehle).
**Baaki:** M6 funds surface, M7 shim retirement. LIVE = NOT_CONFIGURED (correct).

## 2026-09-06 - Phase 21: UBL M4 — explicit broker selection, single source of truth

**Kya hua:** Approved design §8/M4 wire hua. **Naya:** `09_broker/broker/selection_store.py` (`FileSelectionStore`: atomic temp+replace JSON, schema-versioned, fail-closed corrupt/unknown, no secret fields) + `00_app/app/services/broker_selection_service.py` (`BrokerSelectionService`: precedence CLI→persisted→compatibility-default; `select()` validates vs registry, unknown→SelectionError + previous preserved; `surface_allowed`/`broker_choices`/`broker_capabilities`). **CLI:** `--broker <id>` (invalid → rc=2, no fallback). **Bootstrap:** selection service injected/created, `DownloadSettings.provider` DERIVED from selection, historical face via registry, capability-absent → `unavailable_provider(reason)` fail-closed (no silent zerodha fallback). **LIVE tab:** broker pill from selection ONLY when trading-capable, else honest NOT_CONFIGURED + reason; `selection` block added; 5 gates untouched/backend-authoritative. **check-live:** probe name from selection (sandbox still fails ACCOUNT_CONFIRMED — sandbox≠live). **--paper:** selection line printed, behavior identical. **Data panel:** registry-backed `Broker` selector (pure view: choices in, `broker_selected` out, validation/persist in bootstrap; ✓/✗ capability display, rejection shows reason, previous kept).
**Tests:** +36 (selection_store 8, service 9, cli 7, integration 5, selector UI 5, arch-rules 2: selection-state app-level-only + live-gates-authoritative). **1188 total**, 28/28 partitions, ruff/format/pyright/validators clean.
**Compatibility:** Paper/Sandbox/gates/strategy/backtest untouched; `build_provider`/`resolve_broker` shims retained (documented); `Provider`/`BrokerAdapter` got `@runtime_checkable` for fail-closed face checks.
**Measured:** impact 70.2s, full pytest 109.7s, run_tests 227.2s; task wall 5962.7s (~99.4 min); 3 first-run failures repaired (test instance bug, SIM102, pyright dict-narrow) → first-pass False; humans 0.
**Baaki:** M5 adapter package move, M6 funds surface, M7 shim retirement. LIVE = NOT_CONFIGURED (correct).

## 2026-09-06 - Phase 20: UBL M1→M3 implemented (unified broker layer, behavior-preserving)

**Kya hua:** Approved `broker_layer_design.md` ke M1-M3 execute. **M1:** `09_broker/broker/` — `vocab.py` (unified `ErrorCode` StrEnum + typed errors + legacy translation table), `capabilities.py` (`CapabilitySet` algebra, 3 domains × subcaps, funds capability included, legacy trading/market-data maps), `faces.py` (`HistoricalFace`/`MarketDataFace`/`TradingFace`/`BrokerPlugin` protocols + `StaticPlugin`/`FactoryPlugin`). **M2:** `registry.py` — `BrokerRegistry` (the ONLY name→plugin map; duplicate/unknown fail-closed) + `BrokerRecord`; Paper/Sandbox/Zerodha-history registry se expose. **M3:** `selection.py` (`BrokerSelection` validation + `SelectionStore`/`MemorySelectionStore` + pure `surface_resolution` fail-closed matrix — NO UX/M4) + **delegation shims**: `data.provider.factory` aur `execution.broker.factory` ab unified registry se resolve karte hain — same instances (fresh per call), same exceptions/messages, sentinel identity preserved (`is` checks), PAPER downgrade + 5-gate semantics untouched, capability-absent fail-closed (`paper`→history refused, unknown names → legacy message shape).
**Wiring:** pyproject (pyright/testpaths/coverage/wheel + `09_broker`), validate_imports (`broker` domain: consumes nothing; only data/execution/app may import), validate_structure (10 domains), run_tests (28 partitions), gate-coverage test, benchmark impact maps.
**Tests:** 35 new (`09_broker/broker/tests/` — contracts/shims/architecture-rules; AST-level proof: core/research chapters never import broker, koi broker-name branching nahi, exactly 1 registry, ErrorCode single-source). `test_provider.py` cleanup path registry-based (1 legacy assertion updated). **1152 total passed**, 28/28 partitions, ruff/format/pyright/validators clean.
**Compatibility evidence:** PaperBroker/SandboxBroker untouched (koi edit nahi un files par), historical behavior same (test_provider 16/16), `--paper`/`--check-live`/live gates untouched (00_app tests green), no network/credentials/real broker.
**Measured:** impact validation 105.3s (clocked), full pytest 68.9s (1152), run_tests 134.6s (28/28), task wall 5368.7s (~89.5 min). Repairs: 5 first-run failures (2 test bugs, 2 factory-signature, 1 literal refactor + 1 legacy test cleanup) — first-pass False, sab fix + re-verify. Human interventions 0.
**Baaki (M4+):** selection UX + engine wiring (M4), adapter package move (M5), funds surface (M6), shim retirement (M7). Real broker: abhi bhi NOT_CONFIGURED.

## 2026-09-06 - Phase 19: Unified Broker Layer — architecture audit + contract design (STOP before implementation)

**Kya hua:** Sirf audit + design, zero product code. `90_brain/broker_layer_design.md` — current dual broker-boundary map (`02_data` Provider vs `08_execution` BrokerAdapter, file:line evidence), 8 duplication/conflict findings (D1 dual registries/selection, D2 3 capability vocabularies, D3 dual credential systems, D4 dual error vocabularies, D5 3 uncoordinated selection sources, D6 data-vendor vs broker naming, D7 NotConfiguredError inconsistency, D8 no funds surface), UBL target (09_broker: vocab/capabilities/faces/registry/selection/credentials), capability model (3 domains × subcaps), single-selection source-of-truth (fail-closed: missing selection → no history + PAPER), 7-step migration plan (M3 shims = semantic pivot), next-phase file list, risks, 7 required test groups. Paper/Sandbox preserved verbatim; strategy/backtest/risk untouched; core broker-free.
**Proof:** 1117 tests RC=0, ruff/format/pyright/validators clean. Speed journal: first-pass True, 0 repairs, 0 human interventions, wall 1877.8s (31.3 min).
**Decision baaki (implementation se pehle):** M4 selection UX — explicit first-run selection vs `zerodha` default as recorded default.
...
**Release done (same day):** user ke explicit order par sab push — commit `a3a5d65` (live execution stack) + tag `v1.9.0` (`ed68c28` par, as-is) dono `origin` par live. Pehla push attempt network reset par gira, retry par `65480bf..a3a5d65` green. GitHub Release OBJECT baaki hai (`gh` token invalid — user `gh auth login` kare ya tag page se 1-click release banaye).
