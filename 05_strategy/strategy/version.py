...
def restore_version_source(
    strategy_id: str, version_id: str, data_dir: Path | str | None = None
) -> str:
    """Return exact historical source for (strategy_id, version_id). Raises if missing or tampered."""  # noqa: E501
    v = load_version(strategy_id, version_id, data_dir)
    if v is None:
        raise FileNotFoundError(f"version not found: {strategy_id}/{version_id}")
    if not v.source:
        # Backward compat: try to recover from metadata if old file
        raise FileNotFoundError(
            f"historical source not stored for {strategy_id}/{version_id} — cannot restore"
        )
    # Verify hash matches stored source
    recomputed = _hash_text(v.source)
    if recomputed != v.source_hash:
        raise ValueError(
            f"tamper detected: stored source_hash {v.source_hash} does not match recomputed {recomputed} for {strategy_id}/{version_id}"  # noqa: E501
        )
    return v.source
