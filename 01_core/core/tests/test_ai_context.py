...
def test_render_contains_component_and_capability_ground_truth() -> None:
    ...
    assert "historical_data" in rendered
    assert "historical_data.download" in rendered


def test_system_state_reports_live_counts_and_gaps() -> None:
    context = make_builder().build()
    assert "components: 2" in context.sections["system_state"]
    assert "capabilities: 7" in context.sections["system_state"]
...
def test_dependencies_are_explicit() -> None:
    ...
    assert "depends on core, market" not in context.sections["dependencies"]


def test_workflows_are_enumerated() -> None:
    context = make_builder().build()
    assert "data_pipeline" in context.sections["workflows"]
    assert "historical_data.download" in context.sections["workflows"]
