"""Execution History — immutable snapshots + ordered events + deterministic replay.

Generic, strategy-agnostic, file-based. One execution = one JSON file.
"""
...
class ExecutionHistory:
    """Full execution record: snapshot + ordered events + signals."""
    ...
    def from_dict(data: dict[str, Any]) -> ExecutionHistory:
        ...
        return ExecutionHistory(snapshot=snap, events=events, signals=signals)
...
def create_snapshot(
    ...
    execution_id = f"EXEC-{datetime.now(timezone.utc).strftime('%Y%m%d')}-{uuid.uuid4().hex[:6].upper()}"
...
def replay_execution(
    ...
    # We need to reconstruct BarView sequence as original did: slice_bars already done, but we have bars
    ...
    for idx in range(warmup, len(bars)):
        bar = bars[idx]
        # State is flat for this test; for more complex we would track position, but for signal comparison we use flat
        view = BarView(
    ...
    if len(replay_signals) != len(expected_signals):
        ...
        if first_div is None:
            ...
            if len(replay_signals) > len(expected_signals):
                first_div = {
                    ...
                    "expected": None,
                    "actual": replay_signals[len(expected_signals)] if len(replay_signals) > len(expected_signals) else None,
                }
            else:
                first_div = {
                    "sequence": len(replay_signals),
                    "expected": expected_signals[len(replay_signals)] if len(expected_signals) > len(replay_signals) else None,
                    "actual": None,
                }
    else:
        for i, (exp, act) in enumerate(zip(expected_signals, replay_signals)):
            if exp != act:
                status = "MISMATCH"
        ...
        if status == "VERIFIED" and len(replay_events) != len(expected_events):
            ...
            # But we will consider signals primary; if signals match, events may differ due to BarProcessed count
            # For now, require exact event count match as well
            # If mismatch, report
            if len(replay_events) != len(expected_events):
                # Find first event divergence
                for i, (exp_e, act_e) in enumerate(zip(expected_events, replay_events)):
                ...
                if status == "VERIFIED" and len(replay_events) != len(expected_events):
                    ...
                    first_div = {
                        ...
                        "expected": expected_events[len(replay_events)].to_dict() if len(expected_events) > len(replay_events) else None,
                        "actual": replay_events[len(expected_events)].to_dict() if len(replay_events) > len(expected_events) else None,
                    }
