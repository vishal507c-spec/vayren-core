...
def forward_sweep(
    ...
    while _c <= sweep_end:
        _chunk_count += 1
        _c = min(_c + timedelta(days=settings.chunk_days), sweep_end) + timedelta(minutes=1)
    total_chunks = max(_chunk_count, 1)
    ...
    while chunk <= sweep_end:
        ...
        end = min(chunk + timedelta(days=settings.chunk_days), sweep_end)
        n += 1
